...
public class MyOwnGroupCardAdapter extends BaseAdapter {
    private ArrayList<HashMap<String, Object>> data;
    private Context ctx;
    public MyOwnGroupCardAdapter(Context ctx, ArrayList<HashMap<String, Object>> data) {
        this.data = data;
        this.ctx = ctx;
    }
    @Override
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        convertView = LayoutInflater.from(ctx).inflate(R.layout.my_own_group_card, parent, false);
        TextView tv_groupName = convertView.findViewById(R.id.my_own_card_groupName);
        tv_groupName.setText(Objects.requireNonNull(data.get(position).get("groupName")).toString());
        TextView tv_groupType = convertView.findViewById(R.id.my_own_card_groupType);
        tv_groupType.setText(Objects.requireNonNull(data.get(position).get("groupType")).toString());
        TextView tv_groupDate = convertView.findViewById(R.id.my_own_group_groupDate);
        tv_groupDate.setText(Objects.requireNonNull(data.get(position).get("groupDate")).toString());
        TextView tv_groupInitiator = convertView.findViewById(R.id.my_own_groupInitiator);
        tv_groupInitiator.setText(Objects.requireNonNull(data.get(position).get("groupInitiator")).toString());
        ProgressBar progressBar = convertView.findViewById(R.id.my_own_group_progress_bar);
